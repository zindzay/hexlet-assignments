...
class AppTest {
    ...
    void testReversedSequence() {
        ...
        CharSequence reversedSequence = new ReversedSequence(s);

        assertThat(reversedSequence.toString()).hasToString(expected);
        assertThat(reversedSequence.length()).isEqualTo(s.length());
        assertThat(reversedSequence.charAt(1)).hasToString("e");
        assertThat(reversedSequence.subSequence(1, 4)).hasToString("edc");

        CharSequence emptySequence = new ReversedSequence("");
        assertThat(emptySequence.toString()).hasToString("");
        assertThat(emptySequence.length()).isZero();
    }
    // END
}
